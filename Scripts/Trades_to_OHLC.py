...
def trades_to_OHLC_per_minute(df,startDate:str):
...
def OHLC_snipper(df,startDate:str):
...
for i in os.listdir(filePath):
    if not i.startswith('.'):
        ...
        if len(df.columns) == 3:
            OHLC = trades_to_OHLC_per_minute(df,startDate)
            OHLC.to_csv(f'{filePath}/OHLC_{i}')


        elif len(df.columns) == 7:
            OHLC = OHLC_snipper(df,startDate)
            OHLC.to_csv(f'{filePath}/OHLC_{i}')
        else:
